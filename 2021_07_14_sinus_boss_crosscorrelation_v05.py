...
"""
We know the frequency of a frame -> therefore we can calculate the periodtime
and the time one degree takes -> then we know the phase delay and can simply subtract
the phase delay from the calculated phase at which an AP is fired.
"""
...
def analyse_file(rec, timeline):
    ...
    return peak_frame, frames_for_analysis, freq_frame, frame_thr, phase_dict
...
"""
def create_dataframe_entries(rec, timeline, date, file, freq_list, AP_count_list, phase_list, delay_list, frame_list, cell_dict, cell_total, file_list):
    peak_frame, frames_for_analysis, freq_frame, frame_thr, phase_dict = analyse_file(rec, timeline)
    for frame, phases in phase_dict.items():
        ...
        for phase in range(0, len(phases)):
            ...
            #each phase value is a list of two values (phase and delayed phase)
            phase = phase_dict[frame][phase][0]
            phase_list.append(phase)
            phase_delayed = phase_dict[frame][phase][1]
            delay_list.append(phase_delayed)
    ...
    return frame_list, freq_list, AP_count_list, phase_list, delay_list, cell_total, file_list
...
def main(cellsheet, directory):
    ...
    for date, files in dictionary.items():
        ...
        for i in range(len(files_oi) ) : 
            ...
            frame_list, freq_list, AP_count_list, phase_list, delay_list, cell_total, file_list = create_dataframe_entries(rec, timeline, date, file, freq_list, AP_count_list, phase_list, delay_list, frame_list, cell_dict, cell_total, file_list)



    df_raw = pd.DataFrame (data = {"cell" : cell_total, "file_num" : file_list, "frame" : frame_list, "frequency" : freq_list, 
                                  "numberAP" : AP_count_list, "phase" : phase_list, "membrane_delay" : delay_list}, dtype = float)   
...
df_delay, df_int_delay = main(cellsheet, directory)           

#df_raw2.to_pickle("/mnt/live1/kmikulik/Analysis/sinus_df_raw_new2.pkl")
df_int_delay.to_pickle("/mnt/live1/kmikulik/Analysis/sinus_df_int_delay_2021_07_14.pkl")
