...
time_delay = ind_delay * 0.02
plt.plot(crosscorr)
plt.savefig("/mnt/live1/kmikulik/plots/crosscorrelation.png")

plt.plot(cur2[0:5000])
plt.plot(vol2[333:5333])
plt.axvline(x=timeline[peak])

plt.show()


plt.plot(cur2[0:1000])
plt.show()
plt.plot(vol2[6.6:1000])
plt.show()



# """
# the same for the first file
# """
# cur2 = rec[0][25]
# vol2 = rec[1][25]
# crosscorr = np.correlate(cur2, vol2, "full")
# min_ind = np.argmax(crosscorr)
# ind_delay = min_ind - 100000
# time_delay = ind_delay * 0.02
# plt.plot(crosscorr)
...
"""
plot membrane delay for the presentation
"""
from scipy.signal import find_peaks
import math

#first we get a smaller interval, so we can see the wave form in higher resolution
#then we plot both voltage and current trace without membrane delay correction
#red lines should indicate the trough (min) of a wave for better visualization

volx = vol2[0:1000]
volxmin = np.where(volx ==  min(volx))[0][0] # get the index where the trace is at its minimum
vol_timemin = volxmin * 0.02 # convert index to time

curx = cur2[0:1000]
curxmin = np.where(curx == min(curx))[0][0] # get the index where the trace is at its minimum
cur_timemin = curxmin * 0.02 # convert index to time


#plot
figx, ax = plt.subplots(2, figsize=(10, 5))
ax[0].plot(timeline[0:1000], curx)
ax[0].axvline(x = cur_timemin, color = "red")
ax[0].set_ylabel("current pulse [pA]")
ax[1].plot(timeline[0:1000], volx)
ax[1].axvline(x = vol_timemin, color = "red")
ax[1].set_xlabel("time [ms]")
ax[1].set_ylabel("membrane potential [mV]")
plt.savefig("/mnt/live1/kmikulik/plots/no_cross_corr.png")



#now we plot the waves again, but this time we correct for the delay factor we calculated with the crosscorrelation function
voli = vol2[333:1333]
volimin = np.where(voli ==  min(voli))[0][0] # get the index where the trace is at its minimum
voli_timemin = volimin * 0.02 # convert index to time

curi = cur2[0:1000]
curimin = np.where(curi == min(curx))[0][0] # get the index where the trace is at its minimum
curi_timemin = curimin * 0.02 # convert index to time


figi, ax = plt.subplots(2, figsize=(10, 5))
ax[0].plot(timeline[0:1000], curi)
ax[0].axvline(x = curi_timemin, color = "red")
ax[0].set_ylabel("current pulse [pA]")
ax[1].plot(timeline[0:1000], voli)
ax[1].axvline(x = voli_timemin, color = "red")
ax[1].set_xlabel("time [ms]")
ax[1].set_ylabel("membrane potential [mV]")
plt.savefig("/mnt/live1/kmikulik/plots/cross_corr.png")
